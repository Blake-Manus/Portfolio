...
function drawToCanvas() {
  ...
  for (var i = 0, numPts = pointsArr.length; i < numPts; i++) {
    ...
    for (var j = 0, numPts = pointsArr.length; j < numPts; j++) {
      ...
      if (getDistance(pointI, pointII) < 175) {
        //If within distance to another point
...
function updateCanvas() {
  for (var i = 0, numPts = pointsArr.length; i < numPts; i++) {
    ...
    if (point.xPos < 0 || point.xPos > canvas.width)
      point.xMovement = -point.xMovement;
    if (point.yPos < 0 || point.yPos > canvas.height)
      point.yMovement = -point.yMovement;
  }
}
